...
| leader-elect-retry-period   | IPAM_LEADER_ELECT_RETRY_PERIOD | 2s                   | Duration the clients should wait between attempting acquisition and renewal of a leadership (duration string).                                                                                                                                 |
...
| leader-elect-id             | IPAM_LEADER_ELECT_ID           |                      | ID used for leader election lock. If not set, falls back to POD_NAME, and if POD_NAME is not set, uses the [hostname](https://github.com/kubernetes/component-helpers/blob/cc328a2915c4129317268270d5de70d24281a617/node/util/hostname.go#L28) |
| leader-elect-namespace      | IPAM_LEADER_ELECT_NAMESPACE    |                      | Leader election lock namespace. If not set, falls back to POD_NAMESPACE, and if POD_NAMESPACE is not set, panics.                                                                                                                              |
